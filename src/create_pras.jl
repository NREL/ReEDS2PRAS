...
function load_objects(ReEDS_data::ReEDSdatapaths, WEATHERYEAR::Int,
    ...
    forced_outage_dict = Dict(forced_outage_data[!,"ResourceType"] .=>
                              forced_outage_data[!,"FOR"])
...
function create_pras_system(regions::Vector{Region}, lines::Vector{Line},
    ...
    pras_lines, pras_interfaces = make_pras_interfaces(sorted_lines,
                                                       interface_reg_idxs,
                                                       interface_line_idxs,
                                                       regions)
    pras_regions = PRAS.Regions{timesteps, PRAS.MW}(get_name.(regions),
    ...
    pras_gens = PRAS.Generators{timesteps, 1, PRAS.Hour, PRAS.MW}(
        get_name.(sorted_gens), get_type.(sorted_gens), capacity_matrix,
        λ_matrix, μ_matrix)
    ##
    ...
    pras_storages = PRAS.Storages{timesteps, 1, PRAS.Hour, PRAS.MW, PRAS.MWh}(
        get_name.(storages), get_type.(storages), stor_charge_cap_array,
        stor_discharge_cap_array, stor_energy_cap_array, stor_chrg_eff_array,
        stor_dischrg_eff_array, stor_cryovr_eff, λ_stor, μ_stor)
    ##
    ...
    gen_stors = PRAS.GeneratorStorages{timesteps, 1, PRAS.Hour, PRAS.MW, PRAS.MWh}(
        gen_stor_names, gen_stor_cats, gen_stor_cap_array,
        gen_stor_dis_cap_array, gen_stor_enrgy_cap_array,
        gen_stor_chrg_eff_array, gen_stor_dischrg_eff_array,
        gen_stor_carryovr_eff_array, gen_stor_inflow_array,
        gen_stor_grid_withdrawl_array, gen_stor_grid_inj_array, gen_stor_λ,
        gen_stor_μ)

    return PRAS.SystemModel(pras_regions, pras_interfaces, pras_gens, gen_idxs,
                            pras_storages, stor_idxs, gen_stors, genstor_idxs,
...
function process_lines(ReEDS_data, regions::Vector{<:AbstractString},
    ...
    converter_capacity_dict = Dict(converter_capacity_data[!, "r"] .=>
    ...
    system_line_naming_data = DataFrames.subset(
        line_base_cap_data, [:r, :rr] => DataFrames.ByRow(keep_line))
    # split-apply-combine b/c some lines have same name convention
    system_line_naming_data = DataFrames.combine(DataFrames.groupby(
        system_line_naming_data, ["r", "rr", "trtype"]), :MW => sum)

    lines_array = Line[]
    for row in eachrow(system_line_naming_data)
        forward_cap = sum(line_base_cap_data[
            (line_base_cap_data.r.==row.r) .&
            (line_base_cap_data.rr.==row.rr) .&
            (line_base_cap_data.trtype.==row.trtype), "MW"])
        backward_cap = sum(line_base_cap_data[
            (line_base_cap_data.r.==row.rr) .&
            (line_base_cap_data.rr.==row.r) .&
            (line_base_cap_data.trtype.==row.trtype), "MW"])
        ...
        if row.trtype!="VSC"
            push!(lines_array,
                  Line(name, timesteps, row.trtype, row.r, row.rr, forward_cap,
                       backward_cap, "Existing", 0.0, 24))
        else
            push!(
                lines_array,
                Line(name, timesteps, row.trtype, row.r, row.rr, forward_cap,
                     backward_cap, "Existing", 0.0, 24, true,
                     Dict(row.r => converter_capacity_dict[string(row.r)],
...
function split_generator_types(ReEDS_data::ReEDSdatapaths, year::Int64)
    ...
    storage_capacity = capacity_data[(findall(in(storage_types),
                                              capacity_data.i)), :]
    thermal_capacity = capacity_data[(findall(!in(vcat(vg_types,
                                                       storage_types)),
                                              capacity_data.i)), :]
...
function process_vg(generators_array::Vector{<:ReEDS2PRAS.Generator},
    ...
    # split-apply-combine to handle differently vintaged entries
    vg_builds = DataFrames.combine(
        DataFrames.groupby(vg_builds, ["i","r"]), :MW => sum)
    for row in eachrow(vg_builds)
        ...
        push!(generators_array, VG_Gen(name, timesteps, region, maximum(profile),
                                       profile, category, "New", gen_for, 24))
    end
...
function process_storages(storage_builds::DataFrames.DataFrame, FOR_dict::Dict,
    ...
    for (idx,row) in enumerate(eachrow(storage_builds))
        ...
        push!(storages_array, Battery(name, timesteps, string(row.r),
                                      string(row.i), row.MW, row.MW,
                                      round(Int, row.MW) * int_duration,
                                      "New", 1, 1, 1, gen_for, 24))
    end
...
function disagg_existing_capacity(eia_df::DataFrames.DataFrame,
    ...
    if DataFrames.nrow(tech_ba_year_existing) == 0
        return [Thermal_Gen("$(tech)_$(pca)_1", timesteps, pca, built_capacity,
                            tech, "New", gen_for, MTTR)]
    end
    ...
    for (idx,built_cap) in enumerate(existing_capacity)
        ...
        gen = Thermal_Gen("$(tech)_$(pca)_$(idx)", timesteps, pca, gen_cap,
                          tech, "Existing", gen_for,MTTR)
        push!(generators_array, gen)
    end

    #whatever remains, we want to build as new capacity
    if remaining_capacity > 0
        add_new_capacity!(generators_array, remaining_capacity,
                          floor.(Int, avg_cap), floor.(Int, max_cap),
                          tech, pca, gen_for, timesteps, year, MTTR)
    end
...
function add_new_capacity!(generators_array::Vector{<:Any}, new_capacity::Int,
    ...
    if existing_avg_unit_cap == 0
        return push!(generators_array,
                     Thermal_Gen("$(tech)_$(pca)_new_1", timesteps, pca,
                                 new_capacity, tech, "New", gen_for, MTTR))
    end
    ...
    for i in range(1, n_gens)
        push!(generators_array,
              Thermal_Gen("$(tech)_$(pca)_new_$(i)", timesteps, pca,
                          per_gen_cap, tech, "New", gen_for, MTTR))
    end
    ...
    if small_remainder > 0
        ...
        push!(generators_array,
              Thermal_Gen("$(tech)_$(pca)_new_$(n_gens+1)", timesteps, pca,
                          small_remainder, tech, "New", gen_for, MTTR))
    end
...
function make_pras_interfaces(sorted_lines::Vector{Line},
    ...
    new_lines = PRAS.Lines{timesteps, 1, PRAS.Hour, PRAS.MW}(
        line_names, line_cats, line_forward_cap, line_backward_cap, line_λ,
        line_μ)

    interface_forward_capacity_array = Matrix{Int64}(undef, num_interfaces,
                                                     timesteps)
    interface_backward_capacity_array = Matrix{Int64}(undef, num_interfaces,
                                                      timesteps)
    ...
    new_interfaces = PRAS.Interfaces{timesteps, PRAS.MW}(
        interface_regions_from, interface_regions_to,
        interface_forward_capacity_array, interface_backward_capacity_array)
