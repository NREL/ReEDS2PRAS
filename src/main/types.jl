...
struct vg_gen <:generator
    ...
    # Inner Constructors
    vg_gen(name = "gen_1", N = 10, rg = "1", cap = zeros(Int,N), type = "dupv", lg = "new", f_or = 0.0, mttr = 24) = new(name,N,rg,cap,type,lg,f_or,mttr)
    vg_gen(s,t,u,v,w,x) = new(s,t,u,v,w,x,0.0,24)
    vg_gen(s,t,u,v,w) = new(s,t,u,v,w,"new",0.0,24)
   
    # Checks
    vg_gen(s,t,u,v,w,x,y,z) = (!(0.0<=y<=1.0) || (z<0)) ? error("FOR and/or MTTR values passed are not allowed") : new(s,t,u,v,w,x,y,z)
    vg_gen(s,t,u,v,w,x,y,z) = !(x in ["old","new"]) ? error("Unidentified legacy passed") : new(s,t,u,v,w,x,y,z)
    vg_gen(s,t,u,v,w,x,y,z) = !(0<=t<=8784) ? error("Check the PRAS timesteps (N) passed.") : new(s,t,u,v,w,x,y,z)
    vg_gen(s,t,u,v,w,x,y,z) = !(length(v) !== t) ? error("The length of the VG time series data should be equal to PRAS timesteps (N)") : new(s,t,u,v,w,x,y,z)
    vg_gen(s,t,u,v,w,x,y,z) = !(w in ["wind-ons","wind-ofs","dupv","upv","csp"]) ? error("Check the type of VG being passed") : new(s,t,u,v,w,x,y,z)
end
...
function get_capacity(gen::GEN) where {GEN <: generator}
...
struct gen_storage <:storage
    ...
    region::String
    type::String
    charge_cap::Vector{Int64}
    discharge_cap::Vector{Int64}
    energy_cap::Vector{Int64}
    inflow::Vector{Int64}
    grid_withdrawl_cap::Vector{Int64}
    grid_inj_cap::Vector{Int64}
    legacy::String
    charge_eff::Float64
    discharge_eff::Float64
    carryover_eff::Float64
    ...
    # Inner Constructors
    gen_storage(name = "stor_1", N = 10, rg = "reg_1",type = "4-hour", c_cap = fill(round(Int,10.0),1,N), dis_cap = fill(round(Int,10.0),1,N), 
                energy_cap = fill(round(Int,40.0),1,N), infl = fill(round(Int,10.0),1,N), g_with_cap = fill(round(Int,10.0),1,N), 
                g_inj_cap = fill(round(Int,10.0),1,N), lg = "new", chr_eff = 0.9, dis_eff = 1.0, cry_eff = 1.0, f_or = 0.0, mttr = 24) = 
                new(name,N,rg,type, c_cap,dis_cap,energy_cap, infl, g_wth_cap, g_inj_cap, lg, chr_eff, dis_eff,cry_eff, f_or, mttr)
    ...
    gen_storage(k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z) = !(all(0.0.<=[v,w,x].<=1.0)) ? error("Invalid charge/discharge/carryover efficiency passed") : new(k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z)
    gen_storage(k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z) = !(all(length.(o,p,q,r,s,t) .== l)) ? error("The length of the time series data associated with the storage should be equal to PRAS timesteps (N)") : new(k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z)
end
...
function get_grid_withdrawl_capacity(stor::gen_storage)
    return stor.grid_withdrawl_cap
end

function get_grid_injection_capacity(stor::gen_storage)
    return stor.grid_inj_cap
end
